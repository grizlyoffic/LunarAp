...
public class MainActivity extends Activity {
    ...
    // Shizuku process for file operations
    private IShizukuFileService fileService;
    
    // UserService for elevated operations
    ...
    private void requestAllPermissions() {
        ...
        // Request SAF for Android/data access
        requestSAFPermission();
    }
    ...
    private void requestSAFPermission() {
        try {
            // For Android 11+, use SAF to access Android/data folder
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                Uri targetUri = Uri.parse("content://com.android.externalstorage.documents/tree/primary%3AAndroid%2Fdata%2Fcom.dts.freefireth");
                ...
                // Try to open Android/data folder
        ...
        try {
            ...
            if (!binderOk) {
                showErrorScreen("Shizuku Not Running", 
                    "Start Shizuku service first, then reopen this app.",
                    true);
            ...
            if (permission == PackageManager.PERMISSION_GRANTED) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    ...
                    if (!savedUri.isEmpty()) {
                        ...
                        showErrorScreen("Folder Access Required",
                            "Android 11+ needs SAF permission.\nTap 'ACCESS GAME FOLDER' below.",
                            true);
                        return;
                    }
                } else {
                    // Older Android, Shizuku alone is enough
                    fileAccessOk = true;
                }
                ...
                showErrorScreen("Shizuku Permission Needed", 
                    "Grant Shizuku permission in the popup, then tap 'TRY AGAIN'.",
                    false);
            }
    ...
    private void showErrorScreen(String title, String desc, boolean showAllButtons) {
        ...
        // Show/hide extra buttons based on need
        View openSettingsBtn = ((LinearLayout) errorScreen).getChildAt(5);
        View safButton = ((LinearLayout) errorScreen).getChildAt(6);
        
        if (openSettingsBtn != null) openSettingsBtn.setVisibility(showAllButtons ? View.VISIBLE : View.GONE);
        if (safButton != null) safButton.setVisibility(showAllButtons ? View.VISIBLE : View.GONE);
        ...
        threadPool.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    // METHOD 1: Direct file write (works on Android 10-)
                    if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.Q) {
                        success = writeFileDirect(configContent);
                    }
                    
                    // METHOD 2: SAF write (Android 11+)
                    if (!success && Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                        success = writeFileViaSAF(configContent);
                    }
                    
                    // METHOD 3: Shizuku elevated shell (all versions fallback)
                    if (!success) {
                        success = writeFileViaShizuku(configContent);
                    }
    ...
    private boolean writeFileViaSAF(String content) {
        try {
            ...
            // Navigate: Android/data/com.dts.freefireth/files/
            DocumentFile androidDir = getOrCreateFolder(rootDoc, "Android");
            if (androidDir == null) {
                // Direct access - maybe we have files folder already
                return writeToDocument(rootDoc, CONFIG_FILE, content);
            }
            
            DocumentFile dataDir = getOrCreateFolder(androidDir, "data");
            if (dataDir == null) return false;
            
            DocumentFile gameDir = getOrCreateFolder(dataDir, "com.dts.freefireth");
            if (gameDir == null) return false;
            
            DocumentFile filesDir = getOrCreateFolder(gameDir, "files");
            if (filesDir == null) return false;
            
            return writeToDocument(filesDir, CONFIG_FILE, content);
        } catch (Exception e) {
            return false;
        }
    }

    private DocumentFile getOrCreateFolder(DocumentFile parent, String name) {
    ...
    private boolean writeToDocument(DocumentFile folder, String fileName, String content) {
        try {
            DocumentFile existing = folder.findFile(fileName);
            if (existing != null) existing.delete();
            
            DocumentFile newFile = folder.createFile("application/json", fileName);
    ...
    private boolean writeFileViaShizuku(String content) {
        try {
            // Use Shizuku to run shell command
            ...
            if (exitCode == 0) {
                // Verify file exists via shell
                Process checkProcess = Runtime.getRuntime().exec(new String[]{"sh", "-c", "test -f '" + TARGET_PATH + CONFIG_FILE + "' && echo 'EXISTS' || echo 'NOT_FOUND'"});
                java.io.BufferedReader reader = new java.io.BufferedReader(new java.io.InputStreamReader(checkProcess.getInputStream()));
                ...
                try {
                    ...
                    if (!deleted && Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                        ...
                        if (!savedUri.isEmpty()) {
                            ...
                            if (rootDoc != null) {
                                DocumentFile existingFile = findFileInDocument(rootDoc, CONFIG_FILE);
                    ...
                    // Shizuku shell delete
                    if (!deleted) {
                        Process process = Runtime.getRuntime().exec(new String[]{"sh", "-c", "rm -f '" + TARGET_PATH + CONFIG_FILE + "'"});
                        process.waitFor();
                        deleted = true;
                    }
    ...
    private DocumentFile findFileInDocument(DocumentFile folder, String fileName) {
        ...
        for (DocumentFile child : children) {
            if (child.isDirectory()) {
                found = findFileInDocument(child, fileName);
                if (found != null) return found;
            }
        }
        return null;
